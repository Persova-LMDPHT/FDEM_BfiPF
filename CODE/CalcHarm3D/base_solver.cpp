...
inline double Base_solver::Scal(double *x, double *y, int n)
{
	...
	#pragma omp parallel shared(x, y) private(i) num_threads(ompC.GetNumberOfThreads())
	{
...
void Base_solver::Cmplx_axpy(std::complex<double> a, double *x, double *y, double *z, int nb)
{
	...
	#pragma omp parallel shared(x, y, z) private(i, t_re, t_im) num_threads(ompC.GetNumberOfThreads())
	{
		#pragma omp for 
		for (i=0; i<nb; i++)
		{
...
void Base_solver::MultCmplxVectVect(int nb, double *a, double *b, double *c)
{
	...
	#pragma omp parallel shared(a, b, c) private(x, y, z) num_threads(ompC.GetNumberOfThreads()) 
	{
		#pragma omp for 
		for (i=0; i<nb; i++)
		{
