 ...
 *                                                                                           
 *  Earlier uploaded: https://github.com/kiselev2013/MTCalc_with_DFP_COCR/blob/master/Code/COCR_FP/block_2x2_solver.cpp                                                        
										 ...
										 double *di_block, double *ggl_block, double *x, double *y, double *y_omp)
{
	...
	int threadNum = ompC.GetNumberOfThreads();
	if (threadNum == 1 || nb < ompC.GetNMinSparseMultMV2x2()) 
	{
													  ...
													  double *di_block, double *df, double *ggl_f, double *ggu_f)
{
	...
	#pragma omp parallel shared(idi, di_block, df, ggl_f, ggu_f) private(i, adr, b, c) num_threads(ompC.GetNumberOfThreads())
	{
		#pragma omp for
		for(i=0; i<nb; i++)
		{
...
int Block_2x2_solver::solve_l_blockdiag(int nb, double *df, double *ggl_f, double *f, double *x)
{
	...
	#pragma omp parallel shared(df, ggl_f, f, x) private(i, ib, ib1) num_threads(ompC.GetNumberOfThreads())
	{
		#pragma omp for
		for(i=0; i<nb; i++)
		{
...
int Block_2x2_solver::solve_u_blockdiag(int nb, double *df, double *ggu_f, double *f, double *x)
{
	...
	#pragma omp parallel shared(df, ggu_f, f, x) private(i, ib, ib1) num_threads(ompC.GetNumberOfThreads())
	{
		#pragma omp for
		for(i=0; i<nb; i++)
		{
