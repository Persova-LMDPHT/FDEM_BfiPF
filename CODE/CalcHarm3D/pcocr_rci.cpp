 ...
 *                                                                                                                 
 *  Earlier uploaded: https://github.com/kiselev2013/MTCalc_with_DFP_COCR/blob/master/Code/COCR_FP/pcocr_rci.cpp                                                                             
...
int PCOCR_RCI::Run()
{
	...
	case 1:
		...
		*out = r;
		...
		break;
	case 2:
		#pragma omp parallel for private(i) num_threads(ompC.GetNumberOfThreads())
		for(i=0; i<n; i++)
		{
			r[i] = pr[i] - r[i];
		}
