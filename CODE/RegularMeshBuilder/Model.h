...
// Names of material properties
	...
	public:
		...
		PhysicalValue(const PhysicalValue &value)
		{
		...
		inline bool Equal(PhysicalValue &value, bool compareTensors = false)
		{
	...
	public:
		...
		PhysicalValue SigmaNH;
		PhysicalValue SigmaNV;
		PhysicalValue Eps;
		PhysicalValue Mu;
		...
		void CopyPhysicals(const Material &material)
		{
			this->SigmaH = material.SigmaH;
			this->SigmaV = material.SigmaV;
			this->SigmaNH = material.SigmaNH;
			this->SigmaNV = material.SigmaNV;
		...
		bool Equal(Material &material)
		{
			if (!material.SigmaH.Equal(SigmaH)) return false;
			if (!material.SigmaV.Equal(SigmaV)) return false;
			if (!material.SigmaNH.Equal(SigmaNH)) return false;
			if (!material.SigmaNV.Equal(SigmaNV)) return false;
		...
		PhysicalValue& GetValue(PhysicalValueName name)
		{
			switch (name)
			{
			case PhysicalValueName::SigmaNH:
				return SigmaNH;
			case PhysicalValueName::SigmaNV:
				return SigmaNV;
