...
void SmoothOutput2D::Output_dArdz(double *_v2s,double *_v2c,double *results,double *resultc,int OutType,vector<int> &RecToSourceB)
{
	int i,j,k,m,it,node0,node1,node2,node3;
	double ss,lss,vs,sc,lsc,vc,hr,hz,psi;
	int ipls;
	double *v2s,*v2c;
	...
	for(k=0;k<nrec;k++)
	{
			...
			for(it=0;it<2;it++)
			{
				if((!it && OutRecList[k].oz.ves) || (it && OutRecList[k].oz.ves!=1.0))
				{
					...
					for(j=0;j<3;j++)
					{
						...
						vs=(-(1.0-psi)*v2s[node0]-psi*v2s[node1]+(1.0-psi)*v2s[node2]+psi*v2s[node3])/hz;
						vc=(-(1.0-psi)*v2c[node0]-psi*v2c[node1]+(1.0-psi)*v2c[node2]+psi*v2c[node3])/hz;
		...
		else
		{
			...
			ss=(-(1.0-psi)*v2s[node0]-psi*v2s[node1]+(1.0-psi)*v2s[node2]+psi*v2s[node3])/hz;
			sc=(-(1.0-psi)*v2c[node0]-psi*v2c[node1]+(1.0-psi)*v2c[node2]+psi*v2c[node3])/hz;
		}
...
void SmoothOutput2D::Output_Er(double *_v2as, double *_v2ac, double *_v2vs, double *_v2vc, double *results, double *resultc, int OutType, vector<int> &RecToSourceE)
{
	int i, j, k, l, m, it, node0, node1, node2, node3;
	double ss, lss, vs, sc, lsc, vc, hr, hz, psi, eta;
	int ipls;
	...
	for (k = 0; k<nrec; k++)
	{
			...
			for (it = 0; it<2; it++)
			{
				if ((!it && OutRecList[k].oror.ves) || (it && OutRecList[k].oror.ves != 1.0))
				{
					...
					for (j = 0; j<3; j++)
					{
						...
						for (l = 0; l<4; l++)
						{
							vs += OutRecList[k].oror.coef[j + it][l] * v2as[rect[m].nodes[l] - 1];
						}
						...
						vs = (-(1.0 - eta)*v2vs[node0] + (1.0 - eta)*v2vs[node1] - eta*v2vs[node2] + eta*v2vs[node3]) / hr;
						...
						for (l = 0; l<4; l++)
						{
							vc += OutRecList[k].oror.coef[j + it][l] * v2ac[rect[m].nodes[l] - 1];
						}
						...
						vc = (-(1.0 - eta)*v2vc[node0] + (1.0 - eta)*v2vc[node1] - eta*v2vc[node2] + eta*v2vc[node3]) / hr;
		...
		else
		{
			...
			ss -= (-(1.0 - eta)*v2vs[node0] + (1.0 - eta)*v2vs[node1] - eta*v2vs[node2] + eta*v2vs[node3]) / hr;
			sc -= (-(1.0 - eta)*v2vc[node0] + (1.0 - eta)*v2vc[node1] - eta*v2vc[node2] + eta*v2vc[node3]) / hr;
		}
...
void SmoothOutput2D::Output_Er(double *v2as, double *v2ac, double *v2vs, double *v2vc, double *results, double *resultc, int OutType)
{
	int i, j, k, l, m, it, node0, node1, node2, node3;
	double ss, lss, vs, sc, lsc, vc, hr, hz, psi, eta;
