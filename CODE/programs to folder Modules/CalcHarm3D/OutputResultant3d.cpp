 ...
 *
 *                       FDEM_BfiPF 
 *  This file contains code of functions for smoothing 3D EM field
 *
...
int OutputResultant3d::Output(int itime,int ipls,int icmp,vector<Res3DValueType> &vvt)
{
	int i, j, k, l,n_sub, lip, lic;
	double var;
	int ret;
	vector<double> x;
	bool flstcmp;
	...
	{	
		...
		mainSub.AsmGlobalVector(ipls,icmp);
	
		if(ipls==Subdomain::npls-1 && icmp==mainSub.ncmp-1)
		{
			mainSub.prds.solve_nrhs(mainSub.ncmp*Subdomain::npls,mainSub.pr,mainSub.x);

			for(lic=0;lic<mainSub.ncmp;lic++)
			{
				for(lip=0;lip<Subdomain::npls;lip++)
				{
					memcpy((char *)(&(x[0])),(char *)(mainSub.x+(lic*Subdomain::npls + lip)*mainSub.n_nodes_c),
						mainSub.n_nodes_c*sizeof(double));

					mainSub.CalcValuesAll_Node(&(x[0]));
					for (i=0; i<mainSub.n_elem; i++)
					{
						...
						for (j=0; j<sz; j++)
						{
							...
							TaskCalcMesh->SaveResult(vvt[lic]/*ValueType*/,var,pnt,0,lip);
						}
					}
				}
			}
	...
	if(ipls==Subdomain::npls-1 && flstcmp)
	{
		x.clear();
	}
