...
int main()
{
	...
	SobjB.Output_dAzdr(A[0],A[1],B[0],B[1],1,RecToSourceB);
	...
	SobjE.Output_Ez(H[0],H[1],V[0],V[1],E[0],E[1],1,RecToSourceE);

	ofp.open("e2d_ez");
	ofp << scientific << setprecision(14);
	for (i = 0; i<npntE; i++)
	{
		ofp<<0.0<<' '<<0.0<<' '<<(E[0][2*i+1]-E[0][2*i])<<' ';
		ofp<<0.0<<' '<<0.0<<' '<<(E[1][2*i+1]-E[1][2*i])<<'\n';
	}
	ofp.close();
	ofp.clear();
	...
	if (npntE)
	{
		ofps.open("e_s_ez.dat", ios::binary);
		ofpc.open("e_c_ez.dat", ios::binary);

		for (ipls = 0; ipls < npls; ipls++)
		{
			SobjE.Output_Ez(H[0] + task.kpnt*ipls, H[1] + task.kpnt*ipls, V[0] + task.kpnt*ipls, V[1] + task.kpnt*ipls, E2d[0], E2d[1], 1);
			...
			retp = FindElementsForReceivers(npntE * 2, taskout.qr, taskout.qz, taskout.rm, taskout.zm, taskout.reg, recE, vRecOutData, taskout.pnt, taskout.rect, task.sigma, rmin, zmin, rmax, zmax);
			if (retp)
			{
			...
			for (i = 0; i<npntE; i++)
			{
				...
				{
					ofps<0.0<0.0<(E[0][2*i+1]-E[0][2*i]);
					ofpc<0.0<0.0<(E[1][2*i+1]-E[1][2*i]);
				}
				else
				{
					ofps<0.0<0.0<0.0;
