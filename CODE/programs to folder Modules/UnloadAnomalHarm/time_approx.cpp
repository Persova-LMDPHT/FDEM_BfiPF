 ...
 *  
 *                       FDEM_BfiPF 
 *  This file contains code for calculating nonstationary 3D VFEM task
 *  
...
void Time_approx_for_vfem::AddWithRegularEdge(int iedge,vector<int> &isEdgesAnomal)
{
	int j;
	isEdgesAnomal[iedge]=true;
}

void Time_approx_for_vfem::GetRegularEdges(int iedge,vector<int> &re,int &size_re)
{
	int j;
	re[size_re]=iedge;
	size_re++;
...
int Time_approx_for_vfem::Unload_anomal_nodes(char *fname)
{
	...
	for(i=0;i<d->kpar;i++)
	{
		...
		if(d->isTensor[d->nvkat[i]])
		{
			for(m=0;m<3 && !TensorAnomal;m++)
			{
				for(j=0;j<3 && !TensorAnomal;j++)
				{
					TensorAnomal=(fabs(d->sigmaTensor[d->nvkat[i]].val[m][j]-d->sigma0Tensor[d->nvkat[i]].val[m][j])>eps);
				}
			}
		}

		if ((!d->isTensor[d->nvkat[i]] && fabs(d->sigma0[d->nvkat[i]]-d->sigma3d[d->nvkat[i]])>eps) || TensorAnomal)
		{
			elemRenum[i] = k;
			isElemAnomal[i] = true;
			...
			for(j=0;j<12;j++)
			{
				AddWithRegularEdge(Tmap->ed[i][j],isEdgesAnomal);
			}
	...
	{
		nnn=0;
		for(i=Tmap->n_c;i<Tmap->n;i++)
		{
			if(!isEdgesAnomal[i])
			{
				size_re=0;
				GetRegularEdges(i,re,size_re);
				for(j=0;j<size_re;j++)
				{
					if(isEdgesAnomal[re[j]])
					{
						AddWithRegularEdge(i,isEdgesAnomal);
						nnn++;
						break;
					}
				}
...
int Time_approx_for_vfem::Unload_anomal_nodesA0(char *fname)
{
	...
	for(i=0;i<d->kpar;i++)
	{
		if (fabs(d->mu0[d->nvkat[i]]-d->mu3d[d->nvkat[i]])>eps*MU_0)
		{
			isElemAnomal[i] = true;
