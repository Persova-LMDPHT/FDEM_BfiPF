...
namespace ReadWrite
{
	...
	int ReadObjects(char *file_name, vector<Model::GeoBox<double>> &objects)
	{
		...
		try
		{
			...
			{
				fscanf(file_in, "%lf%lf%lf%lf%lf%lf%lf%lf"
					...
					, &newMaterial.SigmaV.constantValue);
				newMaterial.SigmaNH.constantValue = newMaterial.SigmaH.constantValue;
				newMaterial.SigmaNV.constantValue = newMaterial.SigmaV.constantValue;
	...
	int ReadZSig2D(char *file_name, vector<Model::GeoLayer<double>> &layers)
	{
		...
		try
		{
			...
			for (int layerIndex = 0; layerIndex < layersCount; layerIndex++)
			{
				...
				newMaterial.SigmaNH.constantValue = sigma;
				newMaterial.SigmaNV.constantValue = sigma;
			...
			if (inf) 
			{
				...
				{
					inf >> tmpd >> tmpd >> tmpd;
					layers[layerIndex + 1].materialValue.SigmaV.constantValue = 1.0 / tmpd;
					layers[layerIndex + 1].materialValue.SigmaNV.constantValue = 1.0 / tmpd;
					inf >> tmpd >> tmpd;
				}
				inf.close();
			}
	...
	// Write material properties to file
	int WriteProperty3D(char *fileName, vector<Model::Material> &materials, Model::PhysicalValueName prop1)
	{
		FILE *outputFile = NULL;
		try
		{
			...
			for (int i = 0; i < materials.size(); i++)
			{
				val3d = materials[i].GetValue(prop1).constantValue;
				if (prop1 == Model::PhysicalValueName::Mu || prop1 == Model::PhysicalValueName::Eps)
				{
					val2d = val3d;
				}
				else
				{
					if (prop1 == Model::PhysicalValueName::SigmaH)
						val2d = materials[i].SigmaNH.constantValue;
					else
						val2d = materials[i].SigmaNV.constantValue;
				}
	...
	{
		if (WriteProperty3D("Sig3d", materials, Model::PhysicalValueName::SigmaH) != 0)
		{
			write_to_log("Could not write 'Sig3d'\n");
			return 1;
		}

		if (WriteProperty3D("Sig3dZ", materials, Model::PhysicalValueName::SigmaV) != 0)
		{
			write_to_log("Could not write 'Sig3dZ'\n");
			return 1;
		}

		if (WriteProperty3D("dpr3D", materials, Model::PhysicalValueName::Eps) != 0)
		{
			write_to_log("Could not write 'dpr3D'\n");
			return 1;
		}

		if (WriteProperty3D("mu3D", materials, Model::PhysicalValueName::Mu) != 0)
		{
	...
	int WriteMaterials2D(vector<Model::Material> &materials)
	{
		...
		if (WriteProperty2D("SigmaZ", materials, Model::PhysicalValueName::SigmaV) != 0)
		{
