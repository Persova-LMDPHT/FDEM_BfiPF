...
namespace Processing
{
		...
		try
		{
			for (int materialIndex = 0; materialIndex < objectMaterials.size(); materialIndex++)
			{
				if (!(objectMaterials[materialIndex].SigmaH.Equal(layerMaterials[materialIndex].SigmaH) &&
					objectMaterials[materialIndex].SigmaV.Equal(layerMaterials[materialIndex].SigmaV)))
				{
					objectMaterials[materialIndex].SigmaNH.constantValue = layerMaterials[materialIndex].SigmaH.constantValue;
					objectMaterials[materialIndex].SigmaNV.constantValue = layerMaterials[materialIndex].SigmaV.constantValue;
				}
			}
			return 0;
		}
	...
	int MainProcedure()
	{
		...
		receivers.clear();
		if (ReadWrite::ReadReceivers("xyzVectorE", receivers) != 0)
		{
			if (ReadWrite::ReadReceivers("../xyzVectorE", receivers) != 0)
			{
				write_to_log("Error : MainProcedure : Could not read xyzVectorE\n");
				return 1;
			}
		}
		if (ReadWrite::ReadReceivers("xyzVectorB", receivers) != 0)
		{
			if (ReadWrite::ReadReceivers("../xyzVectorB", receivers) != 0)
			{
		...
		if (ReadWrite::ReadSettings("RegularMeshBuilderSettings.cfg", mesh3DSettings, mesh2DSettings) != 0)
		{
			if (ReadWrite::ReadSettings("../RegularMeshBuilderSettings.cfg", mesh3DSettings, mesh2DSettings) != 0)
			{
