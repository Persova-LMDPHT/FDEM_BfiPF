...
LocalMatrixRZ::LocalMatrixRZ(double r0, double r1, double z0, double z1, double mu, double sigma, double sigmaZ, double beta)
{
	this->r0 = rk = r0;
	this->r1 = r1;
	this->z0 = zk = z0;
...
//------------------------------------------------------------------------
// Calculate the local matrix from integrals with Psi*d_Psi/d_z of the finite element
//------------------------------------------------------ 
...
{
	double lmdr[4][4],lmdz[4][4],lmij;

	CalcLocalMatrixB();

	GetStiffnessMatrixDr(r1, z1, r0, z0, lmdr);
	GetStiffnessMatrixDz(r1, z1, r0, z0, lmdz);
	...
	for (i=0; i<4; i++)
	{
		for (j=0; j<4; j++)
		{
			lmij=lmdr[i][j]+lmdz[i][j]*(2.0-sigmaZ/sigma);

			ah[i*3  ][j*3  ][0] =  lmij/mu;
			ah[i*3  ][j*3+1][0] =  0.0;
...
void GetStiffnessMatrixDz(const double &r0, const double &z0,const double &r3, const double &z3,double m[4][4])
{
	double h1=r3-r0, h2=z0-z3, rk=r0;
	double r11, r12, r22, z11, z12, z22;
	double rp11, rp12, rp22;
...
void GetStiffnessMatrixDr(const double &r0, const double &z0,const double &r3, const double &z3,double m[4][4])
{
	double h1=r3-r0, h2=z0-z3, rk=r0;
	double r11, r12, r22, z11, z12, z22;
	double rp11, rp12, rp22;
