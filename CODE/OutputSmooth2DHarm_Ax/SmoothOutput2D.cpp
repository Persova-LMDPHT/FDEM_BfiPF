						 ...
						 int p_nreg,int p_qr,int p_qz,int *p_reg,double *p_rm,double *p_zm)
{
	...
	{
		#pragma omp for private(i,j,k,l,m,ii,jj,ef,iii,jjj,celm)
		for (jjj = 0; jjj < rec_qz; jjj++)
		{
			for (iii = 0; iii < rec_qr; iii++)
			{
				...
				ef = !(rec[k].r > celm.r);
				GetSmoothForR(i, j, k, ef, l, 0, rec[k], OutRecList[k].orobj.lem, OutRecList[k].orobj.coef, OutRecList[k].orobj.v, OutRecList[k].orobj.ves, OutRecList[k].orobj.fl, 0);
			...
			for(it=0;it<2;it++)
			{
				...
				{					
					...
					for(j=0;j<3;j++)
					{
						...
						for(l=0;l<4;l++)
						{
							v[0]+=OutRecList[k].orobj.coef[j+it][l]*v2s[rect[m].nodes[l]-1];
							v[1]+=OutRecList[k].orobj.coef[j+it][l]*v2c[rect[m].nodes[l]-1];
						}	
						
						ls[0]+=OutRecList[k].orobj.v[it][j]*v[0];
						ls[1]+=OutRecList[k].orobj.v[it][j]*v[1];
					}

					s[0]+=ls[0]*((it)? 1.0-OutRecList[k].orobj.ves : OutRecList[k].orobj.ves);
					s[1]+=ls[1]*((it)? 1.0-OutRecList[k].orobj.ves : OutRecList[k].orobj.ves);
				}
			}
		}
		else
...
{
	int i,j,k,l,m,it,nn0,nn1;
	double ss,sc,lss,lsc,vs,vc,hr,hz,psi,eta;

	#pragma omp parallel for private(i,j,k,l,m,it,nn0,nn1,ss,sc,lss,lsc,vs,vc,hr,hz,psi,eta)
	for(k=0;k<nrec;k++)
	{
		if(OutType && OutRecList[k].orobj.fl)
		{
			...
			for(it=0;it<2;it++)
			{
				...
				{					
					...
					for(j=0;j<3;j++)
					{
						...
						for(l=0;l<4;l++)
						{
							vs+=OutRecList[k].orobj.coef[j+it][l]*v2s[rect[m].nodes[l]-1];
							vc+=OutRecList[k].orobj.coef[j+it][l]*v2c[rect[m].nodes[l]-1];
						}	
		...
		else
		{
			...
			nn0=rect[i].nodes[0]-1;
			nn1=rect[i].nodes[3]-1;

			hr=pnt[nn1].r-pnt[nn0].r;
			hz=pnt[nn1].z-pnt[nn0].z;
			psi=(rec[k].r-pnt[nn0].r)/hr;
			eta=(rec[k].z-pnt[nn0].z)/hz;

			ss=(1.0-psi)*(1.0-eta)*v2s[rect[i].nodes[0]-1]+psi*(1.0-eta)*v2s[rect[i].nodes[1]-1]
				+(1.0-psi)*eta*v2s[rect[i].nodes[2]-1]+psi*eta*v2s[rect[i].nodes[3]-1];
			sc=(1.0-psi)*(1.0-eta)*v2c[rect[i].nodes[0]-1]+psi*(1.0-eta)*v2c[rect[i].nodes[1]-1]
				+(1.0-psi)*eta*v2c[rect[i].nodes[2]-1]+psi*eta*v2c[rect[i].nodes[3]-1];
		}
